...
{
    int ClientAchievements_ShowText(char const* Desc, int charnum, char const* Text) noexcept
	{
		return (GetClientAchievements() == nullptr ? 0 : GetClientAchievements()->ShowText(Desc,charnum,Text));
	}
	const char* ClientAchievements_GetTextEntered() noexcept
	{
		return (GetClientAchievements() == nullptr ? 0 : GetClientAchievements()->GetTextEntered());
	}
