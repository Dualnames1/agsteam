...
void IAGS2Client::RegisterScriptFunctions(IAGSEngine *engine) const noexcept
{
	...
	functions[clientName + "::ShowText^3"] = reinterpret_cast<void*>(ClientAchievements_ShowText);
	functions[clientName + "::GetTextEntered"] = reinterpret_cast<void*>(ClientAchievements_GetTextEntered);
	...
	functions[clientName + "::get_OnSteamDeck"] = reinterpret_cast<void*>(AGS2Client_IsOnSteamDeck);
	functions[clientName + "::get_TextDismissed"] = reinterpret_cast<void*>(AGS2Client_IsTextDismissed);
...
namespace AGS2Client
{
	...
	int AGS2Client_IsTextDismissed() noexcept
	{
		return (GetClient()->IsTextDismissed() != 0);
	}

	int AGS2Client_IsOnSteamDeck() noexcept
	{
		return (GetClient()->IsOnSteamDeck() != 0);
	}
